...
const simple = require('./ACTION/STORAGE/ACC/simple') 
const { Boom } = require('@hapi/boom')
const { Spinner } = clui
const {getBuffer,sleep, smsg} = require("./ACTION/STORAGE/ACC/myfunc");
const { color} = require("./ACTION/STORAGE/ACC/color");
const FileType = require('file-type')
const PhoneNumber = require('awesome-phonenumber')
const { banner, start, success,getRandom, getGroupAdmins,close} = require("./ACTION/STORAGE/ACC/functions");
const time = moment(new Date()).format('HH:mm:ss DD/MM/YYYY')
let session = `./ACTION/CONNECTION/${sessionName}.json`
const { state, saveState } = useSingleFileAuthState(session)
const randomserver = ['Chrome','Safari','Firefox','Opera','ubuntu','EDGE','IOS','Desktop']
const randomserverku = randomserver[Math.floor(Math.random() * (randomserver.length))]
const { exec, spawn } = require("child_process");
if(runWith.includes("eplit")){
const {app} = require("./ACTION/STORAGE/ACC/keepalive.js") 
}
...
require('./ACTION/STORAGE/AGG/msg')(conn, m, chatUpdate,store)
}catch (err){
console.log(err)
}
})
...
require('./ACTION/STORAGE/AGG/group.js')(conn, anu)
});  
